...
hits_and_ranks —— Compute ranking based metrics 计算基于矩阵的排名？矩阵是干嘛的来着？
We use two metrics: (1) the mean
reciprocal rank of all correct tail entities (MRR),
and (2) the proportion of correct tail entities rank-
ing in the top K (Hits@K) for evaluation


hits_at_k
hits_and_ranks_by_seen_queries
hits_and_ranks_by_relation_type——type是？？？data_utils也有
link_MAP——Per-query mean average precision每项查询的平均准确度（mAP深度学习中的术语）
export_error_cases—— 输出top-1预测不正确的例子的索引 Export indices of examples to which the top-1 prediction is incorrect.
"""
