...
"""
图形搜索 决策网络
transit  计算下一个行动分布，基于 (a) KG中的当前节点（实体）和查询关系 (b) 行动历史表示
——当前状态的表示（当前节点和其他观察）。
——MLP多层感知器
——policy_nn_fun(X2, action_space):
——def pad_and_cat_action_space(action_spaces, inv_offset)
初始化路径
加载路径
得到动态动作空间在桶中——我们把不同状态的行动空间(即不同节点的出线集合)根据它们的大小分组到桶中，以节省划线的内存消耗。
之后的函数作用不是很懂
apply_action_masks   应用动作掩码??防止智能体在中途停下来
get_ground_truth_edge_mask  获得真实的边缘掩码？
et_ground_truth_edge_mask_multi
