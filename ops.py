...
def batch_lookup(M, idx, vector_output=True):
    ...
    使用索引idx对矩阵M进行批量查找
    参数：
    M（变量）[batch_size,seq_len]M的每一行是一个独立的群
    idx（变量）[batch-size,sample_size] idx的每一行是一个样本索引的列表
    vector_output： 如果设置，当样本容量为1时，返回一个一维向量
    samples：[batch_size,sample_size]
    ...
    """
    assert 断言函数   assert  表达式
    如果值为真，则程序可继续往下执行，反之python解释器会报AssertionError.
    Often,assertion are placed at the beginning of the function for checking the valid input
    ,and the valid output after the function call.
    """
    ...
    if sample_size == 1 and vector_output:
        ...
        samples = torch.gather(M, 1, idx)   #param：input,dim,index
    return samples
    """
    从原tensor中获取指定的dim和指定index数据
    """
...
def entropy(p):
   ...
   """
   torch.sum(input,list:dim,bool:keepdim=false,dtype=false,dtype=none)   tensor
   input:输入一个tensor
   dim：压缩和的维度，可以是一个列表
   keepdim：求和之后的dim元素个数为1，
   torch.sum矩阵加法
   """

def weighted_softmax(v, w, dim=-1):
    exp_v = torch.exp(v)
    #exp()矩阵中每个元素都做exp运算，输出同形状矩阵
